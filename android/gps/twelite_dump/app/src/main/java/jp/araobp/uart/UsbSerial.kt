...
abstract class UsbSerial(val context: Context, baudrate: Int, dataBits: Byte, stopBits: Byte, parity: Byte, flowControl: Byte) {
    ...
    private val mHandlerThread: HandlerThread = HandlerThread("rx").apply { start() }
    ...
    abstract fun parse(messageFraction: ByteArray, len: Int): Object?

    abstract fun onData(data: Object)

    private fun _rx(messageFraction: ByteArray, len: Int) {
        parse(messageFraction, len)?.let {
            mHandler.post {
                onData(it)
            }
        }
