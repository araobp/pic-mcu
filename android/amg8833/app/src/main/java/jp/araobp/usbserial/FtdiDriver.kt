 ...
 */
abstract class FtdiDriver(val context: Context, baudrate: Int, dataBits: Byte, stopBits: Byte, parity: Byte, flowControl: Byte) {
    ...
    fun tx(message: ByteArray) {
        mFtdiDevice?.let {
            if (it.isOpen) {
                ...
                /*
                Log.d(TAG, "FTDI driver tx:")
                message.forEach {byte ->
                    Log.d(TAG, "   ${byte.toUByte().toString(16)}")
                }
                */
            } else {
                Log.e(TAG, "FTDI driver is not open")
            }
        }
    }
